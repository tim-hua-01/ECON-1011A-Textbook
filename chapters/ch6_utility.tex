...
We know that utility functions are invariant with respect to monotonic transformations. However, this does not mean that we can't find \emph{some} utility function given some set of preferences. Suppose we have a set of consumption data for an agent for $N$ total goods. The data records how much of the they consumed (often called a ``bundle" of goods), which a vector $x \in \mathbb{R}^N_+$, as well as the prices of these goods $p \in \mathbb{R}^N_{++}$. A utility function \vocab{rationalizes} the dataset when a consumer with that utility function will generate the same consumption choices in the data when given the prices and income.
